...
describe("LBTC Price Error Reproduction", () => {
  ...
  test("Step 3: Test LBTC in context of market operations", async () => {
    ...
    try {
      ...
      const lbtcMarket = markets.find(market => 
        market.market.coinType === LBTC_COIN_TYPE
      );
      ...
      if (lbtcMarket) {
        ...
        console.log("Available coin types:", markets.map(m => m.market.coinType));
      }
    } catch (error) {
  ...
  test("Step 4: Test LBTC with other coin types in batch", async () => {
    ...
    try {
      const prices = await getPricesFromPyth(testCoinTypes);
      ...
      testCoinTypes.forEach(coinType => {
        ...
        if (hasPrice) {
          ...
          console.log(`    Price: ${priceData?.price.price}`);
        }
      });
  ...
  test("Step 5: Diagnose the exact error path", async () => {
    console.log("Diagnosing the error path that leads to 'Market price not found'...");
    ...
    try {
      ...
      const lbtcMarket = markets.find(m => m.market.coinType === LBTC_COIN_TYPE);
      
      if (lbtcMarket) {
        console.log("Found LBTC market, testing price chain...");
        
        // Test each step in the price resolution chain
        const { pythPriceFeedIdMap } = await import("../src/utils/priceFeedIds");
        const { getPricesFromPyth } = await import("../src/utils/helper");
        ...
        console.log("LBTC market not available - cannot reproduce error in market context");
  ...
  test("Step 6: Test error handling and recovery", async () => {
    ...
    try {
      const prices = await getPricesFromPyth(mixedCoinTypes);
      
      // Check which ones succeeded
      console.log("Mixed batch results:");
      mixedCoinTypes.forEach(coinType => {
        const hasPrice = prices.has(coinType);
        console.log(`  ${coinType.split("::").pop()}: ${hasPrice ? '✅' : '❌'}`);
      });
